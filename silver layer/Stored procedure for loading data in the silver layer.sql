-- Creating  a stored procedure to automate the data inserting and cleaning to the silver layer 
...
-------------------------------------------------------------------------------------------------------------------------------------
		...
		SELECT
			...
			CASE WHEN sls_du_dt = 0 OR LEN(sls_du_dt) != 8 THEN NULL
				 ELSE CAST(CAST(sls_du_dt AS VARCHAR) AS DATE)
			END AS sls_du_dt,
